...
BEGIN_CONNECTION_MAP(CSourceObj, CCmdTarget) 
    CONNECTION_PART(CSourceObj, IID_ISourceObj, SourceObjConnPt) 
END_CONNECTION_MAP()
...
REFIID CSourceObj::XSourceObjConnPt::GetIID()
{
    return IID_ISourceObj; 
}
...
void CSourceObj::FirePropChanged(long nInt)
{
    ...
    POSITION pos = m_xSourceObjConnPt.GetStartPosition(); 
    LPDISPATCH pDispatch; 
    while(pos != NULL)
    {
        pDispatch = (LPDISPATCH)m_xSourceObjConnPt.GetNextConnection(pos); 
        driver.AttachDispatch(pDispatch); 
        TRY
            driver.InvokeHelper(0, DISPATCH_METHOD, VT_EMPTY, 
                NULL, (BYTE*)(VTS_I4), nInt); 
        END_TRY 
