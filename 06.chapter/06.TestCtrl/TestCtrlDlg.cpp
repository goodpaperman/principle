...
void CTestCtrlDlg::OnBtnConn() 
{
        ...
        SUCCEEDED(m_pDispatch->QueryInterface(IID_IConnectionPointContainer, (LPVOID*)&pContainer)))
    {
        ...
        DWORD dwCookie = 0; 
        if(SUCCEEDED(pContainer->FindConnectionPoint(IID_IEventSet, &pPt)))
        {
...
STDMETHODIMP CTestCtrlDlg::XEventSink::PropChanged(long nInt)
{
    CString str; 
    str.Format("The Property has been changed to %d!",nInt); 
    AfxMessageBox(str); 
    return 1; 
}
